import scrapy
from .utils import get_links_deputados,build_csv, get_csv_header, get_nome, get_info_plenario_comissao, get_data_nascimento, get_gastos_par_gab, get_salario_bruto, get_qtd_viagens
import csv
...
class DeputadosSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        info_deputado.append(build_csv(nome, genero, data_nascimento, info_plen_com, gastos_par_gab, salario, qtd_viagens))
        
        
        
        header = get_csv_header()
        with open('info_deputados.csv', 'w') as file:
            writer = csv.writer(file)
            writer.writerow(header)
        
            writer.writerows(info_deputado)
